...
class ManageJob():
    ...
    @staticmethod
    def logical_delete_job(parent, job_name):
        '''
        删除任务,将任务移动到预删除目录位置
        :param parent:
        :param job_name:
        ...
        try:
            ...
            job_script_file = get_script_path(parent, job_name)
            pre_delete_file = get_pre_delete_conf_path(parent, job_name)
            pre_delete_script = get_pre_delete_script_path(parent, job_name)
    ...
    @staticmethod
    def clear_deleted_jobs(parent, job_name):
        '''
        接受一个包含所有需要删除的文件的列表，并删除所有指定的文件
        :param job_list: 需要删除的文件列表
        :return:
        '''
        try:
            job_deleted_file_path = get_pre_delete_conf_path(parent, job_name)
            job_deleted_script_path = get_pre_delete_script_path(parent, job_name)
    ...
    @staticmethod
    def restore_deleted_job(parent, job_name):
        '''
        ...
        '''
        try:
            deleted_file = get_pre_delete_conf_path(parent, job_name)
            deleted_script = get_pre_delete_script_path(parent, job_name)
            conf_file = get_conf_path(parent, job_name)
            conf_script = get_script_path(parent, job_name)
...
def check_job_file_exists(job_file):
    ...
    if os.path.exists(job_file):
        yes_or_no = tkMessageBox.askyesno('是否覆盖？', '任务文件已经存在，是否需要覆盖，谨慎操作，该操作不可逆！')
...
def scan_all_deleted_job(root_path):
    '''
